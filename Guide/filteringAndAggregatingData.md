...
Pressing the ___Filter___ button in the ___Filter and aggregate species data___ region of the ___MetaTaxAssign___ opens the ___Filter and aggregate species data___ window that allows the data to be filtered and aggregated before exporting to a tab-delimited text file.
...
The results of the analysis are initially stored within  the __Append taxonomic data to a file__ window and are not accessible by the rest of __MetaTaxAssign__. Therefore, to save the data, you must press the __Accept__ button (grey line in Figure 13). This will close the window and the results of this analysis step will replace the data in the __MetaTaxAssign__ program. The data can then be saved to a file by pressing the __Save__ button (purple line in Figure 2) or processed further by other functions on the ___Filter and aggregate species data___ window. Pressing the __Cancel__ button will discard the results and retain the original data.
...
The results of the analysis are stored within  the __Remove data columns__ window and are not accessible to the rest of __MetaTaxAssign__. Therefore, to save the data, you must press the __Accept__ button (grey line in Figure 14). This will close the window and the new data set will replace the original one held by __MetaTaxAssign__. The data can then be saved to a file by pressing the __Save__ button (purple line in Figure 2) or processed further by other functions on the ___Filter and aggregate species data___ window. Pressing the __Cancel__ button will discard the results and retain the original data.

## Renaming column names

Where possible, __MetaTaxAssign__ uses the column headers inherited from the original data. If a file contained no column names, __MetaTaxAssign__ creates them in the same manner as Excel labels columns, i.e., __A__ to __Z__, then __AA__ to __AZ__ and then __BA__ to __BZ__ and so on. Consequently, it may be necessary to modify these names so they are more meaningful or concise.  __MetaTaxAssign__ allows you to edit column names by two methods via the __Rename columns__ window. 
...
## Renaming the column names using a saved list of names created by __MetaTaxAssign__
...
Once you have edited the file of column names, press the __Change__ button (red line in Figure 15) and select the file. These will then be imported and matched to the current column names. When complete, __MetaTaxAssign__ will display a message stating how many columns are in the current read-count matrix, how many were changed and how many data lines were in the imported file. Press the __Yes__ button to cause the current edits to be saved.
